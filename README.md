...
- Capture and display screenshot thumbnails for saved tabs when a session is saved.
...
- Chrome only allows screenshot capture from the visible tab, so saving a session briefly activates each capturable tab and then restores the previously active tabs/window.
